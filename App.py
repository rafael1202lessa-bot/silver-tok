...
try:
    # Puxa todos os itens (ativos e inativos) para o Dev ver
...
if not todos_itens:
    ...
    for item in todos_itens:
        ...
        # Cria uma linha organizada para cada produto
        col_nome, col_status, col_btn = st.columns([2, 1, 1])
        
        with col_nome:
            st.write(f"**{item['nome_produto']}** (💰 {item['preco']} moedas)")
        
        with col_status:
            st.write(f"Status: {status}")
            
        with col_btn:
            # Se o item está ativo, mostra o botão de desativar/remover
            if item["ativo"]:
                if st.button("Remover da Loja", key=f"del_{item['id']}", use_container_width=True):
                    try:
                        # Muda o status para False no Supabase
                        supabase.table("loja_itens").update({"ativo": False}).eq("id", item["id"]).execute()
                        st.success(f"Item '{item['nome_produto']}' removido com sucesso!")
                        st.rerun()
                    except Exception as erro_del:
                        st.error(f"Erro ao remover: {erro_del}")
            else:
                # Se já está inativo, dá a opção de ativar de volta
                if st.button("Ativar Item", key=f"ativar_{item['id']}", use_container_width=True):
                    try:
                        supabase.table("loja_itens").update({"ativo": True}).eq("id", item["id"]).execute()
                        st.success(f"Item '{item['nome_produto']}' reativado!")
                        st.rerun()
                    except Exception as erro_ativar:
                        st.error(f"Erro ao ativar: {erro_ativar}")
        st.write("---")
